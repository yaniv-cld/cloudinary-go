...
func TestUploadParmasBoolean(t *testing.T) {
	params := uploader.UploadParams{
	...
	if err != nil {
		t.Error("Error marshalling %w", err)
	}
	p := uploader.UploadParams{}
	err = json.Unmarshal(paramsBytes, &p)
	if err != nil {
		t.Error("Error unmarshalling %w", err)
	}
	assert.Equal(t, false, p.Overwrite)
	assert.Equal(t, false, p.UniqueFilename)
}
